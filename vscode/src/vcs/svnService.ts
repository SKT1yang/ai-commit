...
export class SvnService implements IVersionControlService {
  ...
  async getStatus(): Promise<SvnStatus> {
    ...
    try {
      const { stdout } = await execAsync(`"${this.svnPath}" status`, {
        cwd: this.workspaceRoot,
      });
  ...
  private async filterIgnoreOnCommitFiles(
    ...
    try {
      ...
      const ignorePatterns =
        workspaceConfig.get<string[]>("ignoreOnCommit") || [];
      ...
      if (allIgnorePatterns.length > 0) {
        ...
        console.log(
          `应用忽略模式过滤: ${allFiles.length} -> ${filteredFiles.length}`,
        );
  ...
  async getCommitReadyChanges(): Promise<SvnStatus> {
    ...
    try {
      ...
      const ignoreOnCommitFiles = new Set(
        changelistInfo.ignoreOnCommitFiles || [],
      );
      ...
      console.log(
        `提交准备变更: ${allFiles.length}个文件 -> ${commitReadyFiles.length}个文件（排除ignore-on-commit）`,
      );
   ...
   */
  private async getChangelistInfo(): Promise<{
    ignoreOnCommitFiles?: string[];
  }> {
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (
          trimmedLine.startsWith("--- Changelist 'ignore-on-commit'") ||
          ...
          trimmedLine.startsWith("--- Changelist '") ||
          ...
          console.log(
            `  -> 在ignore-on-commit内，状态: "${status}", 文件: "${filePath}"`,
          );
          if (status && filePath) {
   ...
   * 获取changelist信息的备用方案
   */
  private async getChangelistInfoFallback(): Promise<{
    ignoreOnCommitFiles?: string[];
  }> {
    try {
  ...
  async getDiff(filePath?: string): Promise<string> {
    try {
      ...
      if (error.message && error.message.includes("UTF-8")) {
        ...
        const fileList = status.changedFiles
          .map((f) => `${this.getStatusDescription(f.status)}: ${f.path}`)
          .join("\n");
        return `文件变更列表:\n${fileList}`;
      }
  ...
  async commit(message: string): Promise<string> {
    try {
      await execAsync(`"${this.svnPath}" commit -m "${message}"`, {
        cwd: this.workspaceRoot,
      });
      return await this.getLatestRevision();
    } catch (error) {
  ...
  // 获取最新版本号
  private async getLatestRevision(): Promise<string> {
    try {
      const { stdout } = await execAsync(
        `"${this.svnPath}" info --show-item revision`,
      );
      return stdout.trim();
    } catch (error) {
      console.error("获取 SVN 版本号失败:", error);
      return "";
    }
  }
  ...
  async getRemoteUrl(): Promise<Record<string, string>> {
    ...
    try {
      ...
      // 查找URL行，格式类似于：URL: https://svn.example.com/repo/trunk
      const urlMatch = stdout.match(/URL:\s+(.*)$/m);
      if (urlMatch) {
        return {
          trunk: urlMatch[1].trim()
        };
      }
      
      // 如果URL是分支或标签，也提取相关信息
      const urlLine = stdout.match(/URL:\s+(.*)$/m);
      const repoRootMatch = stdout.match(/Repository Root:\s+(.*)$/m);
      
      if (urlLine && repoRootMatch) {
        const repoUrl = urlLine[1].trim();
        const repoRoot = repoRootMatch[1].trim();
      ...
      console.error("无法从svn info中获取URL");
      return {};
    } catch (error) {
      console.error("获取 SVN 远程仓库地址失败:", error);
      return {};
    }
  }
}
