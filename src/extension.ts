...
function buildCopilotPrompt(
  ...
  function normalizeLanguage(lang: string | undefined): string {
    ...
    if (["en", "english"].includes(l)) {
      return "en";
    }
    if (["ja", "jp", "日本語"].includes(l)) {
      return "ja";
    }
    if (["ko", "kr", "한국어"].includes(l)) {
      return "ko";
    }
    return l;
  }
...
4. Use English for description`;
  ...
  const statsLine = isZhCN
    ? `文件统计: 新增 ${added} 修改 ${modified} 删除 ${deleted} 重命名 ${renamed} (VCS: ${vcsType})`
    : `File stats: added ${added} modified ${modified} deleted ${deleted} renamed ${renamed} (VCS: ${vcsType})`;
